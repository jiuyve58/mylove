...
var S = {
  init: function () {
    ...
    if (i !== -1) {
      ...
      S.UI.simulate('|#countdown 3|情人节快乐|亲爱的|我会一直在|');
    }
...
S.Shape = (function () {
  var dots = [],
      ...
      cx = 0,
      cy = 0;
//根据绘制区域的大小（通过 S.Drawing.getArea() 获取）调整 cx 和 cy，使形状居中。
  function compensate() {
  ...
  return {
    //将所有未激活的点（dots[d].s === false）随机移动到绘制区域内的新位置。
    ...
    //将点重新排列以形成由 n 定义的新形状（n 是一个包含新形状的尺寸和点位置的对象）。
    //如果新形状需要更多的点，则创建额外的 S.Dot 实例。
    //点会被动画化地移动到它们的新位置，速度可以通过 fast 参数控制。
    //多余的点（如果有）会被随机移动并标记为未激活。
